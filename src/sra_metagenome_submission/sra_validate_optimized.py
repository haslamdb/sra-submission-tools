...
Optimized for handling large files with streaming and parallel processing.
"""
        ...
        try:
            with open(config_file, 'r') as f:
                ...
                if 'performance' in config:
                    PERFORMANCE_CONFIG.update(config['performance'])
                    logger.info(f"Loaded performance settings from {config_file}")
...
def check_files_exist_parallel(df, base_dir=None, max_workers=None):
    """
    Check if all files mentioned in the metadata exist using parallel processing.
        ...
        try:
            ...
            if not exists:
                with lock:
                    ...
                    missing_by_sample[sample_name].append({"column": col, "file": file_path})
            
            return exists
            
        except Exception as e:
            logger.error(f"Error checking file for row {idx}, column {col}: {str(e)}")
            return None
    ...
    for idx, row in df.iterrows():
        for col in existing_columns:
            file_info_list.append((idx, row, col))
    ...
    checked_files = 0
    print(f"\nChecking existence of files using {max_workers} parallel workers...")
        ...
        for future in as_completed(futures):
            try:
                ...
                if checked_files % 10 == 0 or checked_files == total_files:
                    print(f"  Checked {checked_files}/{total_files} potential file references...", end='\r')
                    
            except Exception as e:
                logger.error(f"Error in parallel file check: {e}")
    
    print(f"\n  Completed checking {checked_files} file references")
...
def validate_sample_metadata_chunk(df_chunk, config=None):
    """
    Validate a chunk of sample metadata.
    
    Args:
        df_chunk (pd.DataFrame): Chunk of sample metadata dataframe
...
def validate_sample_metadata(df, config=None):
    """
    Validate sample metadata and fix common issues with support for chunked processing.
    ...
    """
    # Check if we should process in chunks based on size
    if len(df) > PERFORMANCE_CONFIG['chunk_size'] * 2:
        logger.info(f"Large dataframe detected ({len(df)} rows), processing in chunks")
        ...
        for i in range(0, len(df), chunk_size):
            ...
            validated_chunk = validate_sample_metadata_chunk(chunk, config)
            chunks.append(validated_chunk)
        
        validated_df = pd.concat(chunks, ignore_index=True)
    else:
        ...
        # Apply the same validation as chunk processing
        validated_df = validate_sample_metadata_chunk(validated_df, config)
...
def validate_bioproject_metadata_chunk(df_chunk, config=None):
    """
    Validate a chunk of bioproject metadata.
    
    Args:
        df_chunk (pd.DataFrame): Chunk of bioproject metadata dataframe
        config (dict, optional): Configuration with default values
    ...
    # Enhanced validation for collection_date
    if 'collection_date' in validated_df.columns:
        for idx, value in validated_df['collection_date'].items():
            try:
                ...
                elif value != default_values['collection_date']:
...
def validate_bioproject_metadata(df, config=None):
    """
    Validate bioproject metadata and fix common issues with support for chunked processing.
    ...
    if len(df) > PERFORMANCE_CONFIG['chunk_size'] * 2:
        ...
        for i in range(0, len(df), chunk_size):
            chunk = df.iloc[i:i+chunk_size]
            validated_chunk = validate_bioproject_metadata_chunk(chunk, config)
            chunks.append(validated_chunk)
        ...
        # Apply the same validation as chunk processing
        validated_df = validate_bioproject_metadata_chunk(validated_df, config)
        ...
        # If we have valid samples, trim the dataframe
        if 'sample_name' in validated_df.columns:
            valid_samples = validated_df['sample_name'].notna() & (validated_df['sample_name'].astype(str) != '')
            if valid_samples.any():
                last_valid_idx = valid_samples.to_numpy().nonzero()[0][-1]
...
def validate_and_fix_metadata(bioproject_file, sample_file, config_file=None, output_dir=None, file_dir=None, max_workers=None):
    """
    Validate and fix bioproject and sample metadata files with parallel file checking.
    ...
    Args:
        ...
        max_workers (int): Number of parallel workers for file checking
    ...
    if config_file:
        ...
        # Update performance config if specified
        if 'performance' in config:
            PERFORMANCE_CONFIG.update(config['performance'])
    
    # Override max_workers if specified
    if max_workers:
        PERFORMANCE_CONFIG['max_workers'] = max_workers
    ...
    # Check if all files mentioned in metadata exist before proceeding with other validations
    if not sample_df.empty and file_dir:
        all_exist, missing_files, missing_by_sample = check_files_exist_parallel(sample_df, file_dir)
    ...
    # Check for filename consistency between sample and bioproject metadata
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="SRA Metadata Validation Tool (Optimized for Large Datasets)"
    )
    ...
    if args.chunk_size:
        PERFORMANCE_CONFIG['chunk_size'] = args.chunk_size
    ...
    # First check if files exist (for both sample and bioproject metadata)
    if args.file_dir:
        ...
        if args.sample_metadata and args.bioproject_metadata:
            try:
                ...
                # Check files from sample metadata using parallel processing
                all_exist, missing_files, missing_by_sample = check_files_exist_parallel(
                    sample_df, args.file_dir, args.max_workers
                )
    ...
    print(f"Performance: Used {PERFORMANCE_CONFIG['max_workers']} workers for file checking")
    if sample_saved or bioproject_saved:
        print("\nValidated files have been saved to the specified location.")
